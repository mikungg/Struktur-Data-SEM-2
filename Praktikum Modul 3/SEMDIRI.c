...
int main()
{
    ...
    {
        if(avl_find(&words, string[i]) == false)
        {
            avl_insert(&words, string[i]);
        }
        else
        {
            avl_remove(&words, string[i]);
        }
    }

    if(avl_isEmpty(&words))
    {
        printf("Tidak ada yang sendiri");
    }
    else
    {
        printf("Mereka sendirian:");
        inorder(words._root);
    }
}